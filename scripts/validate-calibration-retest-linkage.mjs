...
const migration=fs.readFileSync('drizzle/0011_calibration_retest_linkage.sql','utf8');
...
for (const fragment of [
  ...
  'calibration_retest_record_link_delete_journal',
  "'retest-pair-invalidated'"
]) {
  if (!migration.includes(fragment)) errors.push(`retest migration missing ${fragment}`);
}
if ((migration.match(/SET search_path = pg_catalog/g) ?? []).length<4) {
  errors.push('retest trigger functions must lock search_path');
}
...
console.log('Calibration retest linkage validation passed: hash-only pseudonymous pair foundation and candidate retest schema/consent are fixed while runtime issuance/claim, materialization, collection/export, preregistration and legal approval remain disabled.');
