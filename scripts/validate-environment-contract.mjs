...
if (JSON.stringify(contract.server_environment_variables.PCS_CLIENT_IP_HEADER?.allowed_values) !== JSON.stringify(['cf-connecting-ip','x-vercel-forwarded-for','x-forwarded-for','x-real-ip','fly-client-ip'])) errors.push('PCS_CLIENT_IP_HEADER allowed values drift');
...
for (const secret of ['DATABASE_URL','PCS_RATE_LIMIT_SECRET']) {
  const match=example.match(new RegExp('^' + secret + '=(.*)$', 'm'));
...
console.log('Environment contract validation passed: dev/preview/prod boundaries, server-only configuration, blank-secret example, TLS/database separation requirements, production AI-key prohibition and external-evidence boundaries are explicit.');
