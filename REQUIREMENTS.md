...
- [ ] **PCS-QA-007** Security/privacy checklist before release. *(Automated release-security foundation is now separated from an explicit fail-closed release review record. Runtime AI/secret/public-env/client-artifact/header/CSRF/privacy/dependency/browser controls are marked automated-complete; production TLS/HSTS observation, trusted proxy, DB least privilege, secret store/rotation, environment separation, backup/restore, production logs and external security review remain `pending`.)*
