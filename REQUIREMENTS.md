...
- [ ] **PCS-QA-007** Security/privacy checklist before release. *(Automated release-security foundation now includes runtime AI/secret/public-env/client-artifact/header/CSRF/privacy/dependency/browser controls plus fail-closed production client-IP header selection. Real edge sanitization, production TLS/HSTS observation, DB least privilege, secret store/rotation, environment separation, backup/restore, production logs and external security review remain `pending`.)*
