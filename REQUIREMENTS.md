...
- [ ] **PCS-QA-007** Security/privacy checklist before release. *(Automated foundation now includes a 16-threat machine threat model covering private/public capabilities, CSRF, proxy trust, analytics/share leakage, immutable history, DB privilege, backup resurrection, logs, dependencies/secrets, runtime AI, crawler exposure, calibration consent and container privilege. Partial threats bind directly to canonical production evidence. External security review and real production edge/roles/TLS/secrets/logs/backup/deletion-replay evidence remain pending.)*
...
- [ ] **PCS-OPS-003** Error monitoring/health checks. *(Development foundation includes privacy-safe fixed-schema telemetry, bucket-only Web Vitals, minimal DB readiness, six-class production monitoring contract/runbook, and a deployment probe that checks live health/security/crawler boundaries without exporting diagnostics. Independent production monitoring/provider alert routing/deployment correlation/drill evidence remain pending.)*
...
- [ ] **PCS-OPS-006** Public launch gate complete before announcing v1.0. *(Fail-closed launch/runtime gates now consume a canonical 16-record production evidence registry. Each external/manual/research/release blocker is bound to the exact launch/model-activation key, and a future `complete` status requires inspectable artifact reference, observation time, reviewer, environment and notes. All current evidence remains `pending`; v1 announcement/indexing/validated/population-rarity actions remain blocked.)*
