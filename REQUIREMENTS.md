...
- [ ] **PCS-ANA-003** Privacy-preserving calibration-data export path exists. *(Pre-collection engineering now includes separate consent-purpose/receipt foundations, strict offline `calibration-export-record-v0.1-dev`, and a fail-closed operator-plane persistence foundation: hash-only operator credentials, role bindings, two-person export request state, append-only bounded operator audit storage, pseudonymous calibration-record linkage and withdrawal/session-deletion journal, plus an execute-only offline request/review/approve/reject control workflow. The normal runtime DB role has zero access to all calibration consent/operator-plane tables, dedicated `calibration_records` / `calibration_item_responses` schemas and a pseudonymous completed-record retest-linkage foundation now exist but contain no activated runtime ingest/issue/claim path or collected calibration rows; the retest candidate schema uses a separate draft consent identity and hash-only one-time claim credential; no `/api/calibration` route/raw export materializer exists, and raw DB-to-export materialization, future materializer artifact-lineage/purge-regeneration, production operator provisioning evidence, final retest/baseline consent approval, legal approval and production-environment evidence remain open; row-level calibration purge for pre-journaled privacy targets is now implemented through a two-person execute-only control plane; the Wave JA-01 repository candidate scope is now SHA-256 frozen but not externally preregistered.)*
...
- [ ] Phase 5A — closed beta/calibration collection. *(PRE-COLLECTION FOUNDATION: Wave JA-01 is registration-ready but not externally preregistered; consent receipt persistence, normalized fail-closed answer-storage schema, governance policy, operator-plane audit/request storage and withdrawal deletion journal are implemented while runtime ingest/collection/export remain disabled. Offline operator credential/authentication, execute-only two-person export-control workflow and the exact Wave JA-01 repository scope freeze are implemented, while external preregistration, production operator provisioning, raw materialization plus future artifact purge/regeneration, final consent/legal/environment approval, runtime retest issue/claim UX, actual participants/retest cohort and ambiguity feedback remain pending; two-person row-level privacy purge is implemented.)*
