...
- [ ] **PCS-QA-007** Security/privacy checklist before release. *(Automated release-security foundation now includes fail-closed proxy principal selection, real PostgreSQL least-privilege runtime-role proof, and isolated logical backup/restore rehearsal in addition to existing AI/secret/header/CSRF/privacy/dependency/browser controls. Production role/edge/TLS/secret/log/external-review evidence plus provider restore and independently durable deletion-replay proof remain `pending`.)*
