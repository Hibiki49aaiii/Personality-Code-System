...
- [ ] Phase 5A — closed beta/calibration collection. *(PRE-COLLECTION FOUNDATION: Wave JA-01 is registration-ready but not externally preregistered; consent receipt persistence, normalized fail-closed answer-storage schema, governance policy, operator-plane audit/request storage and withdrawal deletion journal are implemented while runtime ingest/collection/export remain disabled. Offline operator credential/authentication, execute-only two-person export-control workflow and the exact Wave JA-01 repository scope freeze are implemented, while external preregistration, production operator provisioning, raw materialization/purge tooling, final consent/legal/environment approval, runtime retest issue/claim UX, actual participants/retest cohort and ambiguity feedback remain pending.)*
