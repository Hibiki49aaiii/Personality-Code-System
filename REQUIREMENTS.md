...
- [ ] **PCS-QA-007** Security/privacy checklist before release. *(Automated release-security foundation now includes runtime AI/secret/public-env/client-artifact/header/CSRF/privacy/dependency/browser controls, fail-closed production client-IP header selection, and a real PostgreSQL least-privilege runtime-role integration proving DML without DDL/definition-write access. Real production role grants, edge sanitization, TLS/HSTS, secret store/rotation, environment separation, backup/restore, production logs and external security review remain `pending`.)*
