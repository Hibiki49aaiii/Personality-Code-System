...
| PCS-ANA-003 | advanced pre-collection/storage/control/retest-linkage/row-purge foundation / runtime ingest + raw materialization blocked | calibration export + baseline/retest consent/governance + normalized answer-storage + pseudonymous retest linkage + operator auth/execute-only export-control + execute-only two-person privacy purge + Wave JA-01 SHA-256 scope-freeze contracts | validators + PostgreSQL exact-model 147-response storage/finalization + 14–21 day retest pair + withdrawal pair-journal + operator/consent/auth/export/privacy-control least-privilege evidence + row-purge + scope-freeze digest | privacy/research | runtime ingest/retest issue-claim, external preregistration/legal/environment/operator provisioning/raw materializer + artifact purge coupling and actual retest evidence remain; row-level purge is implemented and ordinary analytics never becomes calibration data |
