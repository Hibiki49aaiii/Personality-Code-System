...
| PCS-ANA-003 | advanced pre-collection/storage/control/retest-linkage foundation / runtime ingest + raw materialization blocked | calibration export + baseline/retest consent/governance + normalized answer-storage + pseudonymous retest linkage + operator auth/execute-only two-person export-control + Wave JA-01 SHA-256 scope-freeze contracts | validators + PostgreSQL exact-model 147-response storage/finalization + 14–21 day retest pair + withdrawal pair-journal + operator/consent/auth/control least-privilege evidence + scope-freeze digest | privacy/research | runtime ingest/retest issue-claim, external preregistration/legal/environment/operator provisioning/materializer/purge and actual retest evidence remain; ordinary analytics never becomes calibration data |
