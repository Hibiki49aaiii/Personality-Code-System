...
| PCS-ANA-003 | advanced pre-collection foundation / raw export blocked | calibration export + consent/governance/operator auth + Wave JA-01 SHA-256 scope-freeze contracts | validators + PostgreSQL operator/consent/least-privilege evidence + scope-freeze digest verification | privacy/research | external preregistration/legal/environment/operator provisioning/materializer/purge remain; ordinary analytics never becomes calibration data |
