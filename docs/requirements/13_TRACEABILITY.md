...
| PCS-ANA-003 | advanced pre-collection/storage/control foundation / runtime ingest + raw materialization blocked | calibration export + consent/governance + normalized answer-storage + operator auth/execute-only two-person export-control + Wave JA-01 SHA-256 scope-freeze contracts | validators + PostgreSQL exact-model 147-response storage/finalization + withdrawal cascade + operator/consent/auth/control least-privilege evidence + scope-freeze digest verification | privacy/research | runtime ingest, external preregistration/legal/environment/operator provisioning/materializer/purge/retest linkage remain; ordinary analytics never becomes calibration data |
