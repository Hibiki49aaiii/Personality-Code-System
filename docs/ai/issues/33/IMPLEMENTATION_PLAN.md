...
Base main SHA: `1a584b0a14981c688ec90c3feee41fee42d42be2`.
...
- `drizzle/0012_calibration_retest_linkage.sql`
...
## V2 Integration Revision — after Issue #34 / PR #35

The original Issue #33 branch predated answer-level calibration storage. V2 intentionally rebases the design onto main after `calibration_records` / `calibration_item_responses`.

Changes from the original plan:
- migration number moves from 0011 to 0012;
- baseline/retest foreign keys target `calibration_records(calibration_record_id)`;
- baseline timing comes from `calibration_records.completed_at`, not `anonymous_sessions.completed_at`;
- baseline and retest must both be `status='complete'`;
- scope validation reads the exact stored calibration record tuple;
- consent purpose/version is still checked through `calibration_record_links -> calibration_consent_receipts`;
- owner-session/consent deletion still cascades through record links and invalidates/removes active linkage;
- no duplicate answer storage or diagnostic-session coupling is introduced.

This revision preserves the original privacy model while making the retest pair semantically bound to the dedicated research dataset rather than ordinary diagnostic session state.
