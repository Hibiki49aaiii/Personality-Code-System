...
| Trusted proxy | edge/CDN config showing forwarded address headers are overwritten/sanitized | NOT RUN |
...
- spoofed `X-Forwarded-For` / related client-address headers cannot bypass the trusted edge policy;
