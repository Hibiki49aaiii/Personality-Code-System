...
心理測定上のcalibrationは通常のproduct analyticsとは分離します。将来の参加状態を通常診断と分けるためのversion/purpose付きconsent receipt・回答保存・retest linkage構造は用意されていますが、現在のapplication runtime roleはそれらへアクセスできず、実参加者のcalibration dataset、collection endpoint、raw export jobはいずれも存在しません。withdrawal等で既にprivacy deletion eventが付いたpseudonymous rowに対しては、privacy operatorと別reviewerによるoffline二者確認のrow-level purge経路をrepository上で実装しています。ただしraw export artifact自体は存在せず、将来materializerを追加する場合はartifact lineageとpurge/regenerationを同時に実装する方針です。Wave JA-01の候補version scopeはrepository内でSHA-256付きfreeze済みですが、これは外部preregistrationや利用許可を意味しません。回答レベルのcalibration collection/exportは、最終同意文、明示的参加、保存・削除方針、production operator provisioning、外部preregistered sample/analysis plan、legal/privacy review、実環境分離等が成立するまで有効化しない方針です。
