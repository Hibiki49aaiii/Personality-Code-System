...
| PCS-ANA-003 | consent/answer-storage/export-control/retest-linkage foundation verified / runtime ingest and raw export still blocked | calibration export spec, baseline + retest draft consent, normalized `calibration_records` / `calibration_item_responses`, pseudonymous `calibration_retest_linkages`, baseline v0.1 + candidate retest v0.2 schemas, operator-plane persistence, offline operator auth/control + governance policy | strict raw-response schemas; hash-only operator and retest claim credentials; completed-record 14–21 day pairing; pair invalidation journals both pseudonymous IDs; execute-only auth/control roles; atomic two-person audit; ordinary runtime remains zero-access | calibration/retest validators + token/schema tests + PostgreSQL exact-model storage/retest/withdrawal/operator/control/runtime-role integrations | runtime ingest/retest issue-claim, raw materializer, offline purge, production provisioning, final baseline/retest consent/legal/environment approval remain; repository candidate scope is SHA-256 frozen but not externally preregistered |
...
| Phase 5A planning | registration-ready research plan + consent/operator auth/governance foundation / collection not started | beta protocol + Wave JA-01 prereg draft + consent receipt + governance/operator-plane persistence + offline operator CLI | exact prerequisite status map; tested operator issuance/auth/revocation + execute-only request/review/approve/reject + two-person audit/deletion-journal DB invariants; answer-storage schema present but no collected rows/runtime ingest; ordinary runtime denied; collection/export false | calibration validators + CLI lifecycle/least-privilege + consent/operator-plane/runtime-role integrations | external preregistration, runtime retest issue/claim, production operator provisioning, materializer/purge tooling, approved baseline/retest consent/legal/environment plus actual varied beta/retest sample remain; exact repository wave scope and retest linkage foundation are frozen |
