...
| PCS-LEGAL-001 | advanced factual/engineering draft / legal approval pending | legal disclosure + self-deletion + retention + backup-restore contracts | 30/90/180 deletion behavior executable; restore resurrection risk explicitly modeled; logical restore tested | legal/self-deletion/retention/backup validators + PostgreSQL/browser evidence | independently durable deletion replay, provider backup behavior, final jurisdiction/contact/consent publication remain |
...
| PCS-OPS-001 | advanced repository/runtime contract / deployment evidence pending | `ENVIRONMENT_CONTRACT_v0.1.md`, container runtime + launch gate | explicit dev/preview/prod classes, explicit `PCS_DEPLOYMENT_ENV`, preview-only CI classification, production assessment/indexing fail-closed | environment/runtime/launch validators + Container Package + full CI | actual distinct deployments/databases/domain/TLS remain external |
...
| PCS-OPS-003 | advanced repository foundation / external monitoring pending | `observability-v0.1-dev`, `OBSERVABILITY_RUNBOOK_v0.1.md`, health/analytics contracts | privacy-safe readiness + required independent availability/error/finalization/DB/deploy/CWV monitor classes and incident severities | observability validator + existing health/telemetry E2E | actual provider/probe, alert routing, deployment correlation and drill evidence remain external |
...
| PCS-OPS-005 | advanced repository process / actual production freeze pending | release contract + beta manifest + production model activation gate | explicit exact-version freeze semantics, migration review, rollback and CI checks; activation cannot occur through ordinary deploy | release-operations + model-activation validators | actual Phase 5C production model activation/freeze evidence remains |
| PCS-OPS-006 | fail-closed gate implemented in policy + runtime / launch blocked | public launch gate + deployment gate + robots/global metadata | required Master/phase/external evidence cross-check; production new-session activation and crawler indexing remain blocked while current candidate/public launch are false | launch/runtime validators + domain tests + Chromium launch-gate E2E + full CI | may complete only after all listed production/manual evidence is complete |
...
| PCS-QA-007 | advanced automated foundation / explicit production gate open | security/privacy QA + client-address/DB-role/backup-restore policies | proxy principal fail-closed; restricted DB role tested; isolated logical dump/restore preserves all application-table counts/triggers/immutability | security-release-review + domain + DB-role + backup/restore integrations + full CI | production edge/roles/TLS/secrets/logs/external review and deletion-replay/provider restore evidence remain |
