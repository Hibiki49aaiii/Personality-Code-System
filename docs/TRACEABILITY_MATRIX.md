...
| PCS-ANA-003 | consent/export-schema/operator-plane foundation verified / raw export still blocked | calibration export spec, consent-purpose/receipt, `export-schema-v0.1-dev`, operator-plane migration/schema + governance policy | strict raw-response schema; hash-only operator credentials; role-bound two-person request state; append-only bounded audit; pseudonymous record linkage + withdrawal/delete journal; ordinary runtime has zero calibration-plane access | calibration validators + credential tests + PostgreSQL operator-plane/consent/DB-role integrations | operator-facing auth, raw DB-to-export materializer, offline purge executor, approved consent/legal/frozen scope/environment remain |
...
| Phase 5A planning | registration-ready research plan + consent/operator-plane foundation / collection not started | beta protocol + Wave JA-01 prereg draft + consent receipt + governance/operator-plane persistence | exact prerequisite status map; two-person request/audit/deletion-journal DB invariants; no answer-level calibration rows; ordinary runtime read/write denied; collection/export flags false | calibration validators + operator credential/consent/operator-plane/DB-role integrations | external preregistration, operator auth/materializer/purge tooling, approved consent/legal/environment plus actual varied beta/retest sample remain |
