...
| PCS-ANA-003 | consent-storage foundation verified / runtime export still blocked | `CALIBRATION_EXPORT_SPEC_v0.1.md`, consent-purpose contract, `calibration_consent_receipts`, beta protocol | version/purpose receipt is separate from product analytics; model/locale bound, withdrawal-only, owner-delete cascade; runtime DB role has zero table access and no collection/export surface exists | calibration validator + consent persistence integration + DB least-privilege + user-deletion integration | final approved consent, research retention, operator audit, sample plan, wave scope, environment/legal approval and export implementation remain |
