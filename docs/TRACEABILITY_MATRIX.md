...
| PCS-ANA-003 | consent/answer-storage/export-control foundation verified / runtime ingest and raw export still blocked | calibration export spec, consent-purpose/receipt, normalized `calibration_records` / `calibration_item_responses`, `export-schema-v0.1-dev`, operator-plane persistence, offline operator auth/control + governance policy | strict raw-response schema; one-time hash-only operator credential lifecycle; execute-only auth/control DB roles plus narrow admin role; role-bound request/review/approve/reject state; atomic append-only decision audit; pseudonymous deletion journal; ordinary runtime remains zero-access | calibration validators + CLI secret/scope tests + PostgreSQL operator auth/role lifecycle + execute-only export-control/two-person decision + operator-plane/consent/runtime-role integrations | runtime ingest, raw DB-to-export materializer, offline purge executor, production operator provisioning, approved consent/legal/environment remain; repository candidate scope is SHA-256 frozen but not externally preregistered |
...
| Phase 5A planning | registration-ready research plan + consent/operator auth/governance foundation / collection not started | beta protocol + Wave JA-01 prereg draft + consent receipt + governance/operator-plane persistence + offline operator CLI | exact prerequisite status map; tested operator issuance/auth/revocation + execute-only request/review/approve/reject + two-person audit/deletion-journal DB invariants; answer-storage schema present but no collected rows/runtime ingest; ordinary runtime denied; collection/export false | calibration validators + CLI lifecycle/least-privilege + consent/operator-plane/runtime-role integrations | external preregistration, production operator provisioning, materializer/purge tooling, approved consent/legal/environment plus actual varied beta/retest sample remain; exact repository wave scope is frozen |
