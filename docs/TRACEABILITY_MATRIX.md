...
| PCS-QA-007 | advanced automated foundation / explicit manual gate open | privacy/security requirements + security release QA + client-address policy | production app refuses to guess trusted forwarded header and rejects arbitrary/non-IP-shaped values; TLS/HSTS/edge overwrite/DB least privilege/secret store/backup/log/external review remain pending | security-release-review + client-address domain tests + full CI | real edge/deployment/manual evidence before closure |
