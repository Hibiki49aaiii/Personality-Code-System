...
| PCS-ANA-003 | consent/export-schema/operator-plane/auth foundation verified / raw export still blocked | calibration export spec, consent-purpose/receipt, `export-schema-v0.1-dev`, operator-plane persistence, offline operator auth policy/CLI + governance policy | strict raw-response schema; one-time hash-only operator credential lifecycle; execute-only auth/control DB roles plus narrow admin role; role-bound request/review/approve/reject state; atomic append-only decision audit; pseudonymous deletion journal; ordinary runtime remains zero-access | calibration validators + CLI secret/scope tests + PostgreSQL operator auth/role lifecycle + execute-only export-control/two-person decision + operator-plane/consent/runtime-role integrations | raw DB-to-export materializer, offline purge executor, production operator provisioning, approved consent/legal/environment remain; repository candidate scope is SHA-256 frozen but not externally preregistered |
