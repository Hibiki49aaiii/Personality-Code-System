...
| PCS-ANA-003 | consent + strict offline export-schema foundation verified / runtime export still blocked | calibration export spec, consent-purpose/receipt, `export-schema-v0.1-dev`, `exportRecord.ts`, beta protocol | allowlisted raw response record + exact-scope manifest; unknown/private/linkage/demographic/timing/derived fields rejected; runtime DB consent access and export surface remain absent | calibration protocol/export validators + domain adversarial tests + consent persistence + DB-role/deletion integrations | approved consent/legal/retention/operator/sample plan/wave scope/environment plus actual DB-to-export/audit implementation remain |
...
| Phase 5A planning | pre-collection governance + consent-storage foundation complete / collection not started | beta protocol + consent-purpose contract + consent receipt migration/schema | exact prerequisite status map; no answer-level calibration rows; runtime write/read denied; collection/export flags false | calibration validator + consent persistence/DB-role/deletion integrations | approved consent/legal/environment/research plan plus actual varied beta/retest sample remain |
