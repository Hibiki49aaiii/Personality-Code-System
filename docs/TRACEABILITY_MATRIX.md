...
| PCS-LEGAL-001 | advanced factual draft / legal approval pending | legal disclosure + self-deletion + diagnostic-retention contracts | factual draft now matches bearer-owned deletion and executable 30/90/180-day engineering retention; scheduler/backup caveats explicit | legal/self-deletion/diagnostic-retention validators + PostgreSQL/browser evidence | final jurisdiction/legal review, deployed scheduler, backup restore behavior, contact/consent publication remain |
...
| PCS-QA-007 | advanced automated foundation / explicit production gate open | security/privacy requirements, release QA, client-address + DB-role policies | app proxy principal is fail-closed; exact per-table runtime grant policy tested with real restricted PostgreSQL login; DDL/definition writes denied | security-release-review + client-address domain tests + DB least-privilege integration + full CI | production edge overwrite, actual runtime/admin role grant dump, TLS/HSTS, secrets, backup/log/external review remain |
