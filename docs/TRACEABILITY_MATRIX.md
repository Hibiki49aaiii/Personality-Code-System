...
| PCS-ANA-003 | consent/answer-storage/export-control/retest-linkage/row-purge foundation verified / runtime ingest and raw export still blocked | calibration export spec, baseline + retest draft consent, normalized `calibration_records` / `calibration_item_responses`, pseudonymous `calibration_retest_linkages`, two-person privacy-purge policy, baseline v0.1 + candidate retest v0.2 schemas, operator-plane persistence, offline operator auth/control + governance policy | strict raw-response schemas; hash-only operator and retest claim credentials; completed-record 14–21 day pairing; pair invalidation journals both pseudonymous IDs; execute-only auth/export/privacy-control roles; atomic two-person export and row-purge audit; ordinary runtime remains zero-access | calibration/retest/purge validators + token/schema tests + PostgreSQL exact-model storage/retest/withdrawal/operator/export-control/privacy-purge/runtime-role integrations | runtime ingest/retest issue-claim, raw materializer + artifact-lineage/purge-regeneration coupling, production provisioning, final baseline/retest consent/legal/environment approval remain; row-level purge is implemented and repository candidate scope is SHA-256 frozen but not externally preregistered |
...
| Phase 5A planning | registration-ready research plan + consent/operator auth/governance/row-purge foundation / collection not started | beta protocol + Wave JA-01 prereg draft + consent receipt + governance/operator-plane persistence + offline operator/export/privacy-control CLIs | exact prerequisite status map; tested operator issuance/auth/revocation + execute-only export control + two-person row purge/deletion-journal DB invariants; answer-storage schema present but no collected rows/runtime ingest; ordinary runtime denied; collection/export false | calibration validators + CLI lifecycle/least-privilege + consent/operator-plane/retest/privacy-purge/runtime-role integrations | external preregistration, runtime retest issue/claim, production operator provisioning, raw materializer + artifact purge/regeneration, approved baseline/retest consent/legal/environment plus actual varied beta/retest sample remain; exact repository wave scope and retest linkage foundation are frozen |
...
| PCS-OPS-003 | advanced repository foundation / monitoring + log-provider evidence pending | observability/log contracts + health + deployment probe + evidence registry | fixed-schema app logs and six monitor classes; independent monitoring and log-provider privacy are distinct canonical production evidence | observability/log/probe/evidence validators | provider monitor schedule/alerts/deploy correlation/drill plus log retention/access/enrichment review remain external |
