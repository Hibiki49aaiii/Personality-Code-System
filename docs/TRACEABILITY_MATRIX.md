...
| PCS-OPS-006 | fail-closed gate implemented / launch blocked | `PUBLIC_LAUNCH_GATE_v0.1.md`, `public-launch-gate-v0.1-dev.json` | cross-checks required Master/phase gates plus external/manual evidence; launch actions false | launch-gate validator in CI | may complete only after all listed production/manual evidence is complete |
...
| PCS-QA-007 | advanced automated foundation / explicit manual gate open | `08_PRIVACY_SECURITY.md`, `10_TESTING_QA.md`, `SECURITY_PRIVACY_RELEASE_QA_v0.1.md` | automated controls recorded complete; production TLS/HSTS/trusted-proxy/DB least privilege/secret store/rotation/env separation/backup/log/external review remain pending | security-release-review validator + current full CI | requires real deployment/manual evidence before closure |
